...
public class Player extends Entity
{
	...
	private int binocularSound=0;
	
	//Variaveis do Dash
	public static int dash = 0;
	public static int horizontalDirection = 1, verticalDirection = 0;;
	...
	private void updateAnimationTick() 
	{
		...
		if (animationTick >= animationSpeed) 
		{
			...
			if (playerAction != CROUNCH && playerAction != ZOOM && playerAction != FALLING)
			{
			...
			else if (playerAction == ZOOM)
			{
				...
				if (animationIndex == 1)
				{
					...
					if (binocularSound == 0)
						AudioPlayer.playEffect(AudioPlayer.BINOCULARIN1);
					else if (binocularSound == 1)
						AudioPlayer.playEffect(AudioPlayer.BINOCULARIN2);
					else if (binocularSound == 2)
						AudioPlayer.playEffect(AudioPlayer.BINOCULARIN3);
					else if (binocularSound == 3)
						AudioPlayer.playEffect(AudioPlayer.BINOCULARIN4);
					else if (binocularSound == 4)
						AudioPlayer.playEffect(AudioPlayer.BINOCULARIN5);
					else if (binocularSound == 5)
						AudioPlayer.playEffect(AudioPlayer.BINOCULARIN6);
				}
	...
	private void setAnimation() 
	{
		...
		if (Playing.active==true)
		{
			...
			else if (down && dash==0 && !Playing.getZoomPressed())
			{
				animationSpeed=5;
				playerAction = CROUNCH;
				
			}
			else if(dash==0 && !Playing.getZoomPressed())
			{
				animationSpeed=60;
				playerAction = IDLE;
			}
			else if(!getMoving() && !getJump() && !getInAir() && Playing.getZoomPressed())
			{
				animationSpeed=5;
				playerAction = ZOOM;
			}
	...
	private void loadAnimations() 
	{
		...
		//Matriz dos sprites
		animations = new BufferedImage[12][12];
	...
	{
		//Pausar automaticamente quando afk
		if (Game.timeMilPause > 3600)
		{
			Game.timeMilPause=0;
			Playing.paused=true;
		}
		...
		if (dash < dashMinTime || dash > dashMaxTime)
		{
			...
			if (!left && !right && !inAir && !down && !jump && !up && !Playing.getZoomPressed())
			{
			...
			if (!Playing.getZoomPressed() || getInAir())
			{
		...
		if ((!right && left || !left && right) && !down && 
			...
			(CanMoveHere(hitbox.x - wallDistance, hitbox.y, hitbox.width, hitbox.height, levelData))) && !Playing.getZoomPressed()
			)
		{
			moving = true;
		}
