...
public class PlayerReflection extends Entity
{
	...
	private float lightGravity = gravityValue;
	...
	public PlayerReflection(float x, float y, int width, int height, Player player) 
	{
	...
	private void updateRestartGame() 
	{
		if (restartTrigger)
		{
			hitbox.y = 3102 * Game.SCALE;
			
			if (!IsEntityOnFloorReflection(hitbox, levelData))
			{
			setInAir(true);	
	...
	public void update() 
	{
		...
		if ((player.getHitbox().x > 4224 * Game.SCALE) && (player.getHitbox().x < 4800 * Game.SCALE))
		{
			hitbox.x = player.getHitbox().x;
		}
		else if (player.getHitbox().x >= 4800 * Game.SCALE)
		{
			hitbox.y = 3116 * Game.SCALE;
			airSpeed=0;
		}
		else if (player.getHitbox().x <= 4224 * Game.SCALE)
		{
			hitbox.y = 3102 * Game.SCALE;
			airSpeed=0;
		}
	...
	private void updateDash()
	{
		...
		if (dash > dashMinTime && dash < dashMaxTime && (Playing.active==true))
		{
			...
			//Direções unicas
			if (verticalDirection == 0 && horizontalDirection == 1 && hitbox.x <= player.getHitbox().x)
			{
				playerAction = DASH;
			...
			//Diagonais
			if ((verticalDirection == 1) && horizontalDirection == 1 && hitbox.x <= player.getHitbox().x)
			{
				playerAction = DASH;
	...
	public void render(Graphics g)
	{
		if (active)
			g.drawImage(animations[playerAction][animationIndex],
					...
					(int) (hitbox.y - yDrawOffset +1) - Playing.yLevelOffset, width * flipW, height * (-1), null);
	...
	private void updatePosition() 
	{
		if (dash < dashMinTime || dash > dashMaxTime)
		{
			...
			if (right && !left && !down && Playing.active==true && (hitbox.x <= player.getHitbox().x))
			{
				xSpeed = playerSpeed;
			}
			
			if (left && !right && !down && Playing.active==true && (hitbox.x >= player.getHitbox().x))
			{
				xSpeed = -playerSpeed;
			}
