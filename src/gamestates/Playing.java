...
import static utilz.Constants.PlayerConstants.CROUNCH;
import static utilz.Constants.PlayerConstants.DASH;
...
public class Playing extends State implements Statemethods
{
	...
	public static boolean zoomPressed;
	public static boolean zoomReleasedTrigger=false;
	public static boolean canPressZoom = false;
	
	public static float multiplierLevelOffsetRightLeftVar=100, multiplayerVar=5, multiplierXLevelOffsetRightLeft=multiplierLevelOffsetRightLeftVar;
	public static float xLevelOffsetRightLeft = 0, xLevelOffsetRightLeftMax=490;
	
	public static float multiplierYLevelOffsetRightLeft=multiplierLevelOffsetRightLeftVar;
	public static float yLevelOffsetRightLeft = 0, yLevelOffsetRightLeftMax=220;

	private int leftBorderVar = (int)(0.44 * Game.GAME_WIDTH);
	private int rightBorderVar = (int)(0.54 * Game.GAME_WIDTH);
	private int leftBorder = leftBorderVar;
	private int rightBorder = rightBorderVar;

	private int topBorderVar = (int)(0.355 * Game.GAME_HEIGHT);
	private int bottomBorderVar = (int)(0.615 * Game.GAME_HEIGHT);
	private int topBorder = topBorderVar;
	private int bottomBorder = bottomBorderVar;
	...
	bridge = LoadSave.GetSpriteAtlas("/Bridge.png"),
...
///////////////////////////////////////////////////////////////////////////////
	...
	//Quando começar uma nova jornada
	public static boolean newGame=false;
	
	//Destacar alguns eventos, como a cutscene do dash
	public static int situation = 0;
	
	//Ligar ou desligar o Dash
	public static boolean dashEnable = true;
	
	//Ligar ou desligar o Binoculos
	public static boolean binocular = true;
	
	//Ligar ou desligar o EyeGlass
	public static boolean eyeGlass = true;
...
///////////////////////////////////////////////////////////////////////////////
	...
	private void initClasses()
	{
		...
		playerReflection = new PlayerReflection(4306 * Game.SCALE , 3104 * Game.SCALE, (int)(18 * Game.SCALE), (int)(18 * Game.SCALE), player);
	...
	public void zoom(String direction)
	{
		if (direction == "right" || direction == "left")
			if (multiplierXLevelOffsetRightLeft > 5)
				multiplierXLevelOffsetRightLeft/=multiplayerVar;
		
		if (direction == "up" || direction == "down")
			if (multiplierYLevelOffsetRightLeft > 5)
				multiplierYLevelOffsetRightLeft/=multiplayerVar;
		
		//Direita
		if (direction == "right")
		{
			if (xLevelOffsetRightLeft < xLevelOffsetRightLeftMax)
			{
				xLevelOffsetRightLeft+=multiplierXLevelOffsetRightLeft;
			}
			else
			{
				xLevelOffsetRightLeft=xLevelOffsetRightLeftMax;
			}
		}
		//Esquerda
		if (direction == "left")
		...
		if (direction == "down")
		{
			if (yLevelOffsetRightLeft < yLevelOffsetRightLeftMax)
			{
				yLevelOffsetRightLeft+=multiplierYLevelOffsetRightLeft;
			}
			else
			{
				yLevelOffsetRightLeft=yLevelOffsetRightLeftMax;
			}
		}
		//Cima
		if (direction == "up")
	...
	public void zoomResetVertical()
	{
		multiplierYLevelOffsetRightLeft=multiplierLevelOffsetRightLeftVar;

		if (yLevelOffsetRightLeft > 0.5 || yLevelOffsetRightLeft < -0.5)
			yLevelOffsetRightLeft/=1.125;
		else
		{
			yLevelOffsetRightLeft=0;
		}
	}
	
	public void zoomResetHorizontal()
	{
		multiplierXLevelOffsetRightLeft=multiplierLevelOffsetRightLeftVar;
		
		if (xLevelOffsetRightLeft > 0.5 || xLevelOffsetRightLeft < -0.5)
			xLevelOffsetRightLeft/=1.125;
		else
		{
			xLevelOffsetRightLeft=0;
		}
	}
	
	public void resetZoomVariables()
	{
		multiplierYLevelOffsetRightLeft=multiplierLevelOffsetRightLeftVar;
		multiplierXLevelOffsetRightLeft=multiplierLevelOffsetRightLeftVar;
	}
	
	public void resetZoomToDefaultsSettings()
	{
		multiplierLevelOffsetRightLeftVar=100;
		multiplayerVar=5;
		xLevelOffsetRightLeftMax=490;
		yLevelOffsetRightLeftMax=220;
	}
	
	public void changeZoomSettings(float multiplierLevelOffsetRightLeftVar, 
	float multiplayerVar, float xLevelOffsetRightLeftMax, float yLevelOffsetRightLeftMax)
	{
		this.multiplierLevelOffsetRightLeftVar = multiplierLevelOffsetRightLeftVar;
		this.multiplayerVar = multiplayerVar;
		this.xLevelOffsetRightLeftMax = xLevelOffsetRightLeftMax;
		this.yLevelOffsetRightLeftMax = yLevelOffsetRightLeftMax;
	}
	
	@Override
	public void update() 
	{
		//System.out.println(xLevelOffsetRightLeft + " / " + yLevelOffsetRightLeft);
		
		if (!player.getMoving() && !player.getJump() && !player.getInAir() && player.getDash() == 0 && Playing.active==true)
		{
			int distanceXright = 12;
			int distanceXleft = 6;
			int distanceYup = 12;
			int distanceYdown = 12;

				if (player.getHitbox().x > (2400 - distanceXleft) * Game.SCALE && player.getHitbox().x < (2400 + distanceXright) * Game.SCALE
				 && player.getHitbox().y > (2080 - distanceYup)   * Game.SCALE && player.getHitbox().y < (2080 + distanceYdown)  * Game.SCALE)
				{
					canPressZoom=true;
					
					if (zoomPressed)
					{
						//Multi, Smooth, Distance
						changeZoomSettings(100, 1.15f, 600, 600);
						zoom("right");
						zoom("up");
					}
					else
				...
				else if (player.getHitbox().x > (3360 - distanceXleft) * Game.SCALE && player.getHitbox().x < (3360 + distanceXright) * Game.SCALE
					  && player.getHitbox().y > (1872 - distanceYup)   * Game.SCALE && player.getHitbox().y < (1872 + distanceYdown)  * Game.SCALE)
				{
					canPressZoom=true;
					
					if (zoomPressed)
					{
						changeZoomSettings(100, 1f, 950, 0);
						zoom("right");
					}
					else
					{
				...
				{
					resetZoomToDefaultsSettings();
					canPressZoom=true;
					
					if (player.isRight() && !player.isDown() && !player.isUp() && !player.isLeft() && zoomPressed)
					{
						zoom("right");
						resetZoomVariables();
						zoomResetVertical();
					}
					else if (player.isLeft() && !player.isDown() && !player.isRight() && !player.isUp() && zoomPressed)
					{
						zoom("left");
						resetZoomVariables();
						zoomResetVertical();
					}
					else if (player.isUp() && !player.isDown() && !player.isLeft() && !player.isRight() && zoomPressed)
					{
						zoom("up");
						resetZoomVariables();
						zoomResetHorizontal();
					}
					else if (player.isDown() && !player.isRight() && !player.isLeft() && !player.isUp() && zoomPressed)
					{
						zoom("down");
						resetZoomVariables();
						zoomResetHorizontal();
					...
					//Diagonais
					else if (player.isUp() && player.isRight() && !player.isDown() && !player.isLeft() && zoomPressed)
					{
						zoom("up");
						zoom("right");
						resetZoomVariables();
					}
					else if (player.isUp() && player.isLeft() && !player.isDown() && !player.isRight() && zoomPressed)
					{
						zoom("up");
						zoom("left");
						resetZoomVariables();
					}
					else if (player.isDown() && player.isRight() && !player.isUp() && !player.isLeft() && zoomPressed)
					{
						zoom("down");
						zoom("right");
						resetZoomVariables();
					}
					else if (player.isDown() && player.isLeft() && !player.isUp() && !player.isRight() && zoomPressed)
					{
						zoom("down");
						zoom("left");
						resetZoomVariables();
	...
	private void checkCloseToBorderX() 
	{
		...
		xLevelOffset += xLevelOffsetRightLeft;
	}
	...
	private void checkCloseToBorderY() 
	{
		...
		yLevelOffset += yLevelOffsetRightLeft;
	}
	...
	public void keyReleased(KeyEvent e) 
	{
		if (!player.getCantMove())
		{
			switch (e.getKeyCode()) 
			{
				...
				{
					zoomPressed = false;
					if (!zoomReleasedTrigger)
					{
						AudioPlayer.playEffect(AudioPlayer.BINOCULAROUT);
						zoomReleasedTrigger=true;
					}
					break;
				}
			}
	...
	{
		trampolineOne.draw(g, xLevelOffset, yLevelOffset, player);
		trampolineTwo.draw(g, xLevelOffset, yLevelOffset, player);
		trampolineThree.draw(g, xLevelOffset, yLevelOffset, player);
		trampolineFour.draw(g, xLevelOffset, yLevelOffset, player);
		trampolineFive.draw(g, xLevelOffset, yLevelOffset, player);
		trampolineSix.draw(g, xLevelOffset, yLevelOffset, player);
	}
	...
	private void createSpike()
	{
		...
		spikeFour = new Spike(2768, 2352, 29, 1, player);
		
		spikeFive = new Spike(3296-16, 2352, 2, 1, player);
		
		spikeSix = new Spike(3296, 2352, 2, 1, player);
	...
	public static boolean getZoomPressed()
	{
		return zoomPressed;
	}
}
